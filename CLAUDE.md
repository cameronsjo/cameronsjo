...
- **Two cards, one template.** The template is written in the dark palette; the generator writes `assets/stats.svg` and derives `assets/stats-light.svg` by swapping each hex through `LIGHT_THEME` (Artificer light tokens). README's `<picture>` + `prefers-color-scheme` picks one per the viewer's **GitHub theme**, which GitHub maps onto that media query for README images. A template color missing from `LIGHT_THEME` fails the run before anything is written. The workflow's commit step must `git add` both SVGs.
- **The font is embedded.** `assets/fonts/jetbrains-mono-{400,600,700}.woff2` are ASCII + `·` subsets (OFL, `assets/fonts/OFL.txt`), inlined as base64 `@font-face` rules via `{{FONT_FACES}}`, because camo blocks external font URLs. A label character outside the subset renders in a fallback face; re-subset with the command in `generate-profile.mjs`. Keep `"JetBrains Mono"` first in the `.mono` stack: with `ui-monospace` first, Macs used SF Mono instead.
